...
from models.cc_1 import RCCAModule
from models.co import CoNet
from models.coattention_net import CoattentionNet
...
class MobileSal(nn.Module):
    ...
    def __init__(self, pretrained=True, use_carafe=True,
        ...
        self.fpn = MDecoder([16, 24, 32], [16, 24, 32])
    ...
    def forward(self, input, depth=None, test=True):
        ...
        #depth_features = self.depthnet(depth)
                # Repeat the tensor 3 times along the channel dimension
        #depth_3= depth.repeat(3, 1, 1)
        depth_features = self.depthnet(depth)
        ...
        if depth is not None:
            ...
            depth_pred = None
        #depth 
        features = self.fpn([conv1, conv2, conv3], depth_features[-3])
...
class NDecoder(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.inners = nn.ModuleList()
        for i in range(len(in_channels)):
            # Ensure we don't go out of bounds for in_channels
            self.inners.append(ConvBNReLU(in_channels[i], out_channels[i], ksize=1, pad=0))

        self.fuse = nn.ModuleList()
        for i in range(len(out_channels)):
            self.fuse.append(RCCAModule(out_channels[i]))  # Assuming padding and kernel size
            
                # Additional conv layers to adjust channel size after interpolation
        self.top_down_convs = nn.ModuleList()
        for i in range(len(out_channels) - 1):
            self.top_down_convs.append(ConvBNReLU(out_channels[i + 1], out_channels[i], ksize=1, pad=0))


    def forward(self, features, depth):
        results = []
        # Start from the top-most layer
        stage_result = self.fuse[-1](self.inners[-1](features[-1]), depth)

        results.append(stage_result)
        # Process remaining features in reverse order
        for idx in range(len(features) - 2, -1, -1):
            inner_top_down = F.interpolate(stage_result,
                                            size=features[idx].shape[2:],
                                            mode='bilinear',
                                            align_corners=False)
                        # Adjust the channels of the top-down path to match the lateral path
            inner_top_down = self.top_down_convs[idx](inner_top_down)
            inner_lateral = self.inners[idx](features[idx])
            # Fusion step; ensure addition is done outside ConvBNReLU if it doesn't handle two inputs
...
class MDecoder(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.fuse = nn.ModuleList()
        for i in range(len(in_channels) - 1):
            self.fuse.append(RCCAModule(out_channels[i] // 2))
        self.fuse.append(RCCAModule(out_channels[-1]))


    def forward(self, features, depth):
        results = []
        expected_sizes = [(64, 64), (32, 32), (16, 16), (8, 8)]  # Example expected sizes, adjust as necessary
        # Process the top-most feature first and upsample
        
        stage_result = self.fuse[-1](self.inners_a[-1](features[-1]), depth)  # Top layer doesn't need RCCA input from above 10 320 8 8
        results.append(stage_result)
        ...
        for idx in range(len(features) - 2, -1, -1):
            ...
            upsampled_result = F.interpolate(stage_result, size=inner_lateral.shape[2:], mode='bilinear', align_corners=False)
            inner_top_down = self.inners_b[idx](upsampled_result)

            assert inner_lateral.shape[2:] == expected_sizes[idx], f"Size mismatch at layer {idx}"
