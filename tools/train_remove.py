...
from models import model_remove_idr as net
...
def val(args, val_loader, model, criterion):
    ...
    for iter, batched_inputs in enumerate(val_loader):
        ...
        if args.onGPU and torch.cuda.device_count() > 1:
            ...
            #addBatch method of this object seems to be designed to take a batch of predicted outputs and the corresponding batch of ground truth data (target_var) to compute or store metrics for later evaluation.
        salEvalVal.addBatch(output[:,0,:,:], target_var) # reducing the dimension from four to three (batch_size, height, width).
        if iter % 5 == 0:
...
def log_images(writer, model, data_loader, device):
    ...
    # Log images; you might want to log 'outputs' or another processed version of 'images'
    img_grid = torchvision.utils.make_grid(images)
    writer.add_image('four_validation_images', img_grid)
