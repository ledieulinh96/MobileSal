...
class RCCAModule(nn.Module):
    def __init__(self, in_channels):
        ...
        #self.pre_attention = ConvBNReLU(in_channels, inter_channels, ksize=3, pad=1)
        # self.conva = nn.Sequential(nn.Conv2d(in_channels, inter_channels, 3, padding=1, bias=False),
        #                            InPlaceABNSync(inter_channels))
        self.cca = CrissCrossAttention(in_channels)
        # self.convb = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, 3, padding=1, bias=False),
        #                            InPlaceABNSync(inter_channels))
        #self.post_attention = ConvBNReLU(in_channels, in_channels, ksize=1, pad=1)
...
class RCCAModule(nn.Module):
    def __init__(self, in_channels):
        ...
        self.post_attention = ConvBNReLU(inter_channels, in_channels, ksize=1, pad=0)
