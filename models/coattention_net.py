...
class CoattentionNet(nn.Module):
    ...
    """
    def __init__(self, num_embeddings, width, height, embed_dim=512, k=30):
        super().__init__()
        
                # Assume num_channels = 320, adapt to a typical size for LSTM input, e.g., 512
        ...
        # Placeholder for flattened size, to be initialized in forward
        self.flattened_size = None
        self.question_fc = None  # Initialize this later once size is known
    ...
    def forward(self, image, question):                    # Image: B x 512 x 196
        m_batchsize, _, height, width = image.shape
        
                # Initialize the fully connected layer if not already done
        if self.question_fc is None:
            self.flattened_size = 64 * (height//4) * (width//4)  # Based on the pooling and conv layers defined
            self.question_fc = nn.Linear(self.flattened_size, 512)

        # Process question with conv layers
        question_features = self.question_conv(question)
        question_features = self.question_fc(question_features.view(question_features.size(0), -1))
